...
export default function TopTickerRibbon({
  ...
  const [quotes, setQuotes] = useState<Quote[]>(
    symbols.map((s) => ({ symbol: s, last: null, changePct: null }))
  );
  ...
  async function fetchQuotes() {
    try {
      ...
      const mapped: Quote[] = symbols.map((s) => {
        const d = data[s.toUpperCase()] || {};
        const lastNum = Number(d.last);
        const changeNum = Number(d.changePct);

        return {
          symbol: s.toUpperCase(),
          last: Number.isFinite(lastNum) ? lastNum : null,
          changePct: Number.isFinite(changeNum) ? changeNum : null,
        };
      });

      console.log('Ticker snapshot', mapped); // Debug
      setQuotes(mapped);
    } catch (e) {
  ...
  // Triple data for endless scroll
