...
export default function TopTickerRibbon({
  ...
  const loopData = useMemo(() => [...rows, ...rows, ...rows], [rows]);
    ...
    <div className="sticky top-0 z-40 w-full border-b bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:bg-neutral-950/85 dark:supports-[backdrop-filter]:bg-neutral-950/60">
      <div className="mx-auto max-w-screen-2xl">
        <div className="h-10 overflow-hidden relative">
          <div
            ref={containerRef}
            className="flex gap-6 absolute whitespace-nowrap animate-ticker"
            aria-busy={loading}
            aria-live="polite"
          >
            {loopData.map((item, idx) => {
              ...
              const price = item.price == null ? '—' : (item.symbol === 'BTC-USD' ? `$${item.price.toLocaleString()}` : item.price.toLocaleString());
              return (
